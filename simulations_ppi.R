#===============================================================================
#
#  PROGRAM: simulations_ppi.R
#
#  AUTHORS: Stephen Salerno (ssalerno@fredhutch.org)
#
#  PURPOSE: R script to reproduce the supplemental simulation results under the
#           PPI++ data generating mechanism for the manuscript entitled:
#
...
p_load(ipd, gam, tidyverse, update = F)

#=== HELPER FUNCTIONS ==========================================================

#--- DATA GENERATING MECHANISM -------------------------------------------------

dgm_ppi <- function(true_beta, x1, x2, x3, x4, err) {

  return(true_beta * x1 + x2 + err)
}

#--- EIV -----------------------------------------------------------------------

postpi_eiv <- function(dat_t, dat_l, dat_u, fit_naive, sigma_p, pred_err) {
  ...
  fit_rel <- lm(y ~ pred, data = dat_l)

  gamma1 <- coef(fit_rel)[2]

  lambda <- 1 - (sigma_p^2 / var(dat_l$pred))

  #- Corrected Relationship Model Variance

  var_pred_comp <- lambda^2 * gamma1^2 * pred_err^2

  var_x_comp <- (1 - lambda)^2 * gamma1^2 * (var(dat_l$pred) - pred_err^2)

  var_rel_comp <- sigma(fit_rel)^2 - var_x_comp - var_pred_comp

  #- Corrected Estimator and Standard Error

  X_u <- model.matrix(~ x1, data = dat_u)

  est <- solve(crossprod(X_u)) %*% t(X_u) %*%

    ((coef(fit_rel)[1] / lambda) + (coef(fit_rel)[2] / lambda) * dat_u$pred)

  se <- sqrt(diag(solve(crossprod(X_u)) *

    (var_rel_comp * n_u / n_l + var_x_comp * n_u / n_l +

       var_pred_comp * n_u / n_l + (gamma1)^2 * sigma(fit_naive)^2)))
...
sim_func <- function(nsims = 100, n_t = 100, n_l = 100, n_u = 100, nboot = 100,
  ...
  est_oracle <- est_naive <- est_classic <- est_postpi1 <- est_postpi2 <-
    ...
    est_ppi <- est_plusplus <- est_popinf <- est_postpi3 <- rep(NA, nsims)

  err_oracle <- err_naive <- err_classic <- err_postpi1 <- err_postpi2 <-

    err_ppi <- err_plusplus <- err_popinf <- err_postpi3 <- rep(NA, nsims)
  ...
  for(sim in 1:nsims) {
    ...
    err <- rnorm(n_tot, sd = 1)

    #-- GENERATE OUTCOME FROM THE DATA GENERATING FUNCTION

    y_t <- dgm(true_beta, x1[which(set_ind == "train")],

      x2[which(set_ind == "train")], x3[which(set_ind == "train")],

      x4[which(set_ind == "train")], err[which(set_ind == "train")])

    y_l <- dgm(true_beta, x1[which(set_ind == "lab")],

      x2[which(set_ind == "lab")], x3[which(set_ind == "lab")],

      x4[which(set_ind == "lab")], err[which(set_ind == "lab")])

    y_u  <- dgm(true_beta, x1[which(set_ind == "unlab")],

      x2[which(set_ind == "unlab")], x3[which(set_ind == "unlab")],

      x4[which(set_ind == "unlab")], err[which(set_ind == "unlab")])
    ...
    dat$pred <- true_beta * x1 + x2 + rnorm(n_tot, mean = -2, sd = 1)
    ...
    fit_postpi3 <- postpi_eiv(dat_t, dat_l, dat_u, fit_naive,

      sigma_p = 1, pred_err = 0)

    est_postpi3[sim] <- fit_postpi3$est[2]

    err_postpi3[sim] <- fit_postpi3$se[2]
  }
  ...
  results <- results |>
    ...
    mutate(
      ...
      upr_postpi3  = est_postpi3 + 1.96 * err_postpi3,
      lwr_postpi3  = est_postpi3 - 1.96 * err_postpi3,
      cov_postpi3  = ((upr_postpi3 > true_beta) & (lwr_postpi3 < true_beta)),
      tval_postpi3 = est_postpi3 / err_postpi3,
      pval_postpi3 = 2 * pt(-abs(tval_postpi3), df = (n_u - 2))
    )
...
ggsave("fig_pval_eiv.png", fig_pval, width = 16, height = 8)
...
ggsave("fig_cov_eiv.png", fig_cov, width = 16, height = 8)

#--- FIGURE 3: POINT ESTIMATES UNDER THE ALTERNATIVE ---------------------------
...
ggsave("fig_est_eiv.png", fig_est, width = 16, height = 8)

#--- FIGURE 4: STANDARD ERRORS UNDER THE ALTERNATIVE ---------------------------
...
ggsave("fig_err_eiv.png", fig_err, width = 16, height = 8)
